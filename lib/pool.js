...
function Pool(opts) { // jshint maxcomplexity: 12, maxstatements: 42
    ...
    this.requestTimeout = validNum(opts, 'requestTimeout', Pool.defaults.requestTimeout, true);
...
Pool.defaults = {
    ...
    requestTimeout: 30 * 1000, // no timeout if set to 0
...
Pool.prototype.acquire = function (cb) {
    ...
    var req, afterFulfill = function(resOrErr, cb) {
        req.removeListener('resolve', afterFulfill);
    ...
    req = new ResourceRequest(this.requestTimeout, cb);
    req.on('error', this.emit.bind(this, 'warn', 'Request error:'));
    req.once('reject', afterFulfill);
    req.once('resolve', afterFulfill);
...
Pool.prototype.end = function (cb) {
    ...
    var closeResources = function () {
        if (this.requests.length !== 0 || this.available.length !== this.pool.count() || this.acquiring !== 0) {
            return true;
        }
        this.removeListener('drain', closeResources);
...
Pool.prototype._maybeAllocateResource = function () { // jshint maxstatements: 23, maxcomplexity: 11
    // do nothing if there are no requests to serve
    if (this.requests.length === 0) { return; }

    // call callback if there is a request and a resource to give it
    if (this.available.length || this.ending) {
        ...
        } while (this.requests.length && (req.fulfilled || (this.ending && this.pool.count() === 0 && this.acquiring === 0)));
        
        if (!req || !(!req.fulfilled && res)) {
            if (this.ending && this.requests.length === 0) {
                this.emit('drain');
            } 
            return;
        }
...
Pool.prototype._allocateResource = function () {
    ...
    onError = function (err) {
        ...
        // resource requests should be notified, that there is a problem
        if (this.pool.count() === 0 && this.requests.length !== 0) {
            var req;
            while ((req = this.requests.shift()) !== null) {
                req.reject(err);
            }
        }
    }.bind(this);
