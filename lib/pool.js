...
Pool.prototype._maybeAllocateResource = function () { // jshint maxstatements: 18
    ...
    if (this.available.length) {
        ...
        var abort = function (err) {
            ...
            this.remove(res);
            this._respondToTimedOutRequests(err);
            this._maybeAllocateResource();
        }.bind(this);
        
        var timer = setTimeout(function () {
            debug('Ping timeout, removing resource (id=%s)', getId(res));
            abort(new Error('Timed out pinging resource'));
        }, this.pingTimeout);
...
Pool.prototype._respondToTimedOutRequests = function (err, immediate) {
    if ( this.ending || this.destroyed ) { return; }

    var pendingRequests = this.requests;
    this.requests = new Deque();

    var req;
    while (( req = pendingRequests.pop() )) {
        var requestThreshold = ((new Date()) - req.ts);
        if ( !this.destroyed && requestThreshold > this.requestTimeout ) {
            debug('Responded to timed out requests. (pending=%d)', this.requests.length + pendingRequests.length);
            setImmediate(req.cb, err);
        } else {
            this.requests.unshift(req);
        }
    }
